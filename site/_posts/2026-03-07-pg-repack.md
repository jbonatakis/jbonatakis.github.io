...
Every row in a table in Postgres is stored as a tuple. Tuples are combined into an 8kb container referred to as a page. A page is a range of space within an actual file on disk defined by an offset. These files exist within a directory for the specific table that they make up, and are 1 GB in size. 
...
```
In the above tree, the directory `16384/` represents a table. The files each contain pages, and in those pages are tuples. A page look something like:

```bash
+--------------------------------------------------+
| header | pointers | free space | tuple data      |
+--------------------------------------------------+
...
```bash
+--------------------------------------------------+
| tuple header | id | marker | payload | metadata |
+--------------------------------------------------+
```
